...
# 检查参数个数是否满足要求
if [[ $# -lt 3 ]]; then
    echo "使用错误！使用方法 sh file.sh 要生成的controlplane.yaml文件名 要设置的master节点的主机位 DHCP临时IP地址主机位
    使用示例： sh master.sh 2 103 102"
    exit 1
fi
...
if [ -n "$2" ]; then
    ...
    sed -i "98s/.*/                - 172.16.102.${hostname_path}\/24/" "${new_filename}"
    sed -i "99s/.*/                - 2408:8631:c02:ffa2::${hostname_path}\/64/" "${new_filename}"
...
while true; do
  ...
  service_status=$(talosctl service -n 172.16.102.${hostname_path})
  if [[ $? -ne 0 ]]; then
      echo "获取服务状态失败。"
  fi
  ...
  if [[ "$state_value" == "$expected_state" ]]; then
      ...
      talosctl service -n 172.16.102.${hostname_path}
      break
  else
