...
#if test -f "$sql_netbox"; then
#  echo "文件 /home/k8s/NetBoxCustomFields.csv 存在"
#else
#  echo "文件 /home/k8s/NetBoxCustomFields.csv 不存在 请检查sql文件"
#  exit 1
#fi
...
# 检查参数个数是否满足要求
if [[ $# -lt 2 ]]; then
    echo "使用错误！使用方法 sh ddi.sh 要配置的v4v6，240-244中的主机位第一段 和第二段
    使用示例： sh ddi.sh 240 244"
    exit 1
    else
...
#修改services.yaml文件
sed -i "s/172.16.102.240-172.16.102.244/172.16.102.$one_path-172.16.102.$tow_path/g" "${services_path}"
if [[ $? -ne 0 ]]; then
    echo "修改ipv4文件失败"
    exit 1
fi
sed -i "s/2408:8631:c02:ffa2::240-2408:8631:c02:ffa2::244/2408:8631:c02:ffa2::$one_path-2408:8631:c02:ffa2::$tow_path/g" "${services_path}"
if [[ $? -ne 0 ]]; then
    GREEN="\033[0;32m"
    NC="\033[0m"  # No Color
...
echo "修改core配置文件中的dns，请输入dns主机位"
read dns_path
if [ $? -eq 0 ]; then
    echo "读取命令执行成功"
    echo "修改的的值为: $dns_path"
    else
    echo "读取命令执行失败"
    fi
sed -i "s/172.16.102.240/172.16.102.${dns_path}/g" "${core_path}"
sed -i "s/2408:8631:c02:ffa2::240/2408:8631:c02:ffa2::${dns_path}/g" "${core_path}"

echo "修改core配置文件中的dhcp，请输入dhcp主机位"
read dhcp_path
if [ $? -eq 0 ]; then
    echo "读取命令执行成功"
    echo "修改的的值为: $dhcp_path"
    else
    echo "读取命令执行失败"
    fi
sed -i "s/172.16.102.241/172.16.102.${dhcp_path}/g" "${core_path}"
sed -i "s/2408:8631:c02:ffa2::241/2408:8631:c02:ffa2::${dhcp_path}/g" "${core_path}"
if [ $? -eq 0 ]; then
    echo "修改core配置文件成功"
...
#修改ddi配置文件
echo "修改dii配置文件中的dns，请输入dns主机位"
read ddi_dns_path
if [ $? -eq 0 ]; then
    echo "读取命令执行成功"
    echo "修改的的值为: $ddi_dns_path"
    else
    echo "读取命令执行失败"
    fi
sed -i "s/172.16.102.240/172.16.102.${ddi_dns_path}/g" "${ddi_file_path}"
sed -i "s/2408:8631:c02:ffa2::240/2408:8631:c02:ffa2::${ddi_dns_path}/g" "${ddi_file_path}"

echo "修改ddi配置文件中的dhcp，请输入dhcp主机位"
read ddi_dhcp_path
if [ $? -eq 0 ]; then
    echo "读取命令执行成功"
...
sed -i "s/172.16.102.241/172.16.102.${ddi_dhcp_path}/g" "${ddi_file_path}"
sed -i "s/2408:8631:c02:ffa2::241/2408:8631:c02:ffa2::${ddi_dhcp_path}/g" "${ddi_file_path}"
if [ $? -eq 0 ]; then
...
function configure_postgres() {
    ...
    echo "请输入你要指定的数据库IP地址 主机位！"
    read sql_path
    local db_host="172.16.102.${sql_path}"
    local db_port="32002"
    local db_user="root"
...
printf "\n开始部署协议交换服务，请选择部署模式：（如果单机部署则输入1 如果是多master部署 则直接回车跳过该步骤）\n"
