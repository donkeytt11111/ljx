...
if [ -n "$4" ];then
    ...
    sed -i "s/172.16.102.102/172.16.102.${ipv46_patch}/g" "${ctrl_file}"
    sed -i "s/2408:8631:c02:ffa2::102/2408:8631:c02:ffa2::${ipv46_patch}/g" "${ctrl_file}"
    echo "修改成功" `cat ${ctrl_file} | grep -E "172.16.102|2408:8631:c02:ffa2::"`
    else
    echo "修改IP地址失败"
    exit 1
fi
...
if [ -n "$5" ];then
    echo "开始下发配置文件"
    xiafa=$(talosctl apply-config --insecure -n 172.16.102.${linshi_ip} --file controlplane.yaml)
    echo "下发成功：$xiafa "
    else
    echo "配置文件下发异常"
    exit 1
...
if [ -n "$6" ];then
    ...
    sed -i "s/127.0.0.1/172.16.102.${talos_cfg}/g" "${taloscfg_file}"
    sed -i "5a\        nodes:" "${taloscfg_file}"
    sed -i "6a\            - 172.16.102.${talos_cfg}" "${taloscfg_file}"
    echo "修改成功" `cat ${taloscfg_file} | grep -E "172.16.102"`
    else
    echo "修改talos地址失败"
    exit 1
fi
...
function send_config_files_by_email() {
    ...
    # 定义接收者列表
    recipients=("18742423211@139.com" "1553232697@qq.com" "18640549232@163.com" "1223645860@qq.com" "442283241@qq.com" "2505584859@qq.com")
